...
import { X, ShieldCheck, AlertTriangle, Share2, Flag, Pill } from "lucide-react";
...
interface Props {
  result: {
    barcode:  string;
    product:  { name?: string; brand?: string; ingredients?: string; imageUrl?: string } | null;
    analysis: {
      score:         number;
      grade:         string;
      findings:      Finding[];
...
const DRUG_BG: Record<string, string> = {
  high: "bg-red-50 border-red-200",
  medium: "bg-orange-50 border-orange-200",
  low: "bg-yellow-50 border-yellow-200",
};
const DRUG_FG: Record<string, string> = {
  high: "text-red-700", medium: "text-orange-700", low: "text-yellow-700",
};
...
export default function ResultSheet({ result, onClose }: Props) {
  ...
  const { product, analysis, drugAlerts = [] } = result;
  const color  = GRADE_COLOR[analysis.grade] ?? "#EF4444";
  ...
  async function submitFakeReport() {
    ...
    try {
      ...
      alert("Could not submit report. Check Supabase is configured.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end bg-black/50" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 pb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-gray-900 text-sm truncate">
              {product?.name ?? "Barcode: " + result.barcode}
            </p>
            <p className="text-xs text-gray-400">{product?.brand ?? "Unknown brand"}</p>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 pb-4 flex-wrap">
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg" style={{ background: color + "22" }}>
            <span className="text-xl font-black" style={{ color }}>{analysis.grade}</span>
        ...
        {hasDrug && (
          <div className="mx-4 mb-3">
            {drugAlerts.map((d, i) => (
              <div
                key={i}
                className={"p-3 rounded-xl border mb-2 " + (DRUG_BG[d.severity] ?? "bg-orange-50 border-orange-200")}
              >
                <p className={"text-xs font-bold mb-1 flex items-center gap-1 " + (DRUG_FG[d.severity] ?? "text-orange-700")}>
                  <Pill size={12} />
                  DRUG INTERACTION — {d.drug}
                  <span className="ml-auto uppercase text-[10px]">{d.severity}</span>
                </p>
                <p className={"text-xs leading-relaxed " + (DRUG_FG[d.severity] ?? "text-orange-700")}>
                  {d.message}
                </p>
        ...
        {analysis.findings.length > 0 ? (
          ...
          <div className="mx-4 mb-3 p-3 bg-green-50 border border-green-100 rounded-xl">
            <p className="text-xs font-semibold text-green-700">✓ No flagged additives found</p>
            <p className="text-xs text-green-600 mt-0.5">Based on Phase 1 database (30+ dangerous additives).</p>
          </div>
        )}
        ...
        {(analysis.positives?.length ?? 0) > 0 && (
          <div className="px-4 mb-4">
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mb-2">Nutrients Found</p>
            <div className="flex flex-wrap gap-1.5">
              {analysis.positives!.map(p => (
                <span key={p} className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">{p}</span>
              ))}
            </div>
          </div>
        )}
          ...
          <div className="mx-4 mb-3 bg-gray-50 border border-gray-200 rounded-xl p-4">
            {submitted ? (
              <div className="text-center py-2">
                ...
                <p className="text-gray-500 text-xs mt-1">Thank you for helping protect Nigerian consumers.</p>
                <button onClick={() => { setShowReport(false); setSubmitted(false); }} className="mt-3 text-xs text-gray-500 underline">Close</button>
              </div>
            ) : (
              <>
                <p className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-1.5">
                  <Flag size={14} className="text-red-500" /> Report this product
                </p>
                <input
                  value={reportMkt}
                  onChange={e => setReportMkt(e.target.value)}
                  placeholder="Where did you buy it? (market, shop name)"
                ...
                <textarea
                  ...
                  placeholder="Describe the issue (e.g. packaging different, wrong colour, expired)"
                  rows={3}
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowReport(false)} className="flex-1 py-2.5 rounded-xl text-sm text-gray-500 bg-gray-100">Cancel</button>
                  <button
