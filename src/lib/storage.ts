...
// Version 2 adds family_profiles store. Existing v1 data is preserved on upgrade.
  ...
  try {
    ...
    _db = await openDB("safelens-v1", 2, {
      upgrade(db, oldVersion) {
        // v1 stores (created fresh or from prior install)
        if (oldVersion < 1) {
...
// ── Scan history ──────────────────────────────────────────────
export async function saveScan(id: string, result: unknown) {
  try {
    await (await getDB())?.put("scans", {
      id,
      barcode:   id,            // keep barcode field for ScanHistory display
      result,
      scannedAt: Date.now(),
    });
  } catch {}
...
export async function clearScanHistory() {
  try { await (await getDB())?.clear("scans"); }
  catch {}
}

// ── Legacy single profile (kept for backward compat) ──────────
